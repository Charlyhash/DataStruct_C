...
node *delete_(node *root, char *key);
...
void print_tree(node *root)
{
	...
	while (!p->is_leaf) {
		...
		p = p->next;
		if (!p) {
			p_down = (node*)p_down->pointers[0];  // next level
			p = p_down;
			printf("\n");
		}
	}
...
node *make_new_node()
{
	...
	nd->pointers = (void**)malloc(size * sizeof(void *));
	nd->keys = (char**)malloc((size - 1) * sizeof(char *));
	nd->parent = NULL;
	nd->num_keys = 0;
	nd->is_leaf = false;
	nd->next = NULL;
...
node *make_new_tree(char *key, record *rec)
{
	...
	root = make_new_leaf();
	root->pointers[0] = rec;
	root->keys[0] = (char*)malloc(MAX_KEY_LEN);
...
void insert_into_node(node *nd, node *right, int index, char *key)
{
	...
	nd->keys[index] = (char*)malloc(MAX_KEY_LEN);
...
node *insert_into_node_after_splitting(node *root, node *nd, node *right, int index, char *key)
{
	...
	temp_ps = (node**)malloc((size + 1) * sizeof(node *));
	temp_ks = (char**)malloc(size * sizeof(char *));
...
node *insert_into_leaf_after_splitting(node *root, node *leaf, int index, char *key, record *rec)
{
	...
	temp_ps = (record**)malloc(size * sizeof(record *));
	temp_ks = (char**)malloc(size * sizeof(char *));
	for (i = 0; i < size; i++) {
		if (i == index) {
			temp_ps[i] = rec;
